...
if ( ! class_exists( 'vxcf_form_data' ) ) {
 ...
 KEY lead_id (lead_id),
  ...
  PRIMARY KEY  (id)
		...
		public function update_lead( $update, $insert, $lead_id = '', $lead = [] ) {
				  global $wpdb;
			 $date           = current_time( 'mysql' );
			$lead['updated'] = $date;
			$leads           = $this->get_crm_table_name();
			...
			if ( empty( $lead_id ) ) {
				...
				$wpdb->update( $leads, $lead, [ 'id' => $lead_id ] );
			}
		...
		public function get_entries( $form_id, $per_page = 20, $req = '' ) {
			...
			if ( isset( $req['type'] ) ) {
				$search .= ' and l.type =' . (int) vxcf_form::post( 'type', $req );  }
			if ( isset( $req['user_id'] ) ) {
				$search .= ' and l.user_id =' . (int) vxcf_form::post( 'user_id', $req );  }
			...
			if ( vxcf_form::post( 'order', $req ) != '' ) {
				if ( vxcf_form::post( 'order', $req ) != '' && in_array( vxcf_form::post( 'order', $req ), [ 'asc', 'desc' ] ) ) {
			...
			$start      = max( $start, 0 );
			$sql_fields = 'l.*';
			if ( ! empty( vxcf_form::$sql_select_fields ) ) {
				$sql_fields .= vxcf_form::$sql_select_fields; }
		...
		public function add_time_sql( $search, $req ) {
			...
			switch ( $time_key ) {
				case 'today': 
					$start_date = strtotime( 'today', $time );
					break;
				case 'this_week': 
					$start_date = strtotime( 'last sunday', $time );
					break;
				case 'last_7': 
					  $start_date = strtotime( '-7 days', $time );
					break;
				case 'last_30': 
					  $start_date = strtotime( '-30 days', $time );
					break;
				case 'this_month': 
					  $start_date = strtotime( 'first day of 0 month', $time );
					break;
				case 'yesterday': 
		...
		public function add_note( $note, $id ) {
			...
			if ( ! empty( $id ) ) {
				$wpdb->update( $notes_table, $note, [ 'id' => $id ] );   
			} else {
		...
		public function search_lead_detail( $value, $form_id, $entry_id = '' ) {
			...
			$table        = $this->get_crm_table_name();
			$detail_table = $this->get_crm_table_name( 'detail' );
			$sql          = $wpdb->prepare( " SELECT d.*,l.form_id FROM {$detail_table} d left join {$table} l on(d.lead_id=l.id) where l.form_id=%s and d.value like %s and l.type=0", [ $form_id, '%' . $value . '%' ] );
			if ( ! empty( $entry_id ) ) {
				$sql .= ' and l.id!=' . $entry_id;  }
		...
		public function get_related_leads( $vis_id, $id ) {
				  global $wpdb;
			$table = $this->get_crm_table_name();
			$sql   = $wpdb->prepare( "Select * from {$table} where vis_id=%s and id!=%d order by id desc limit 20", [ $vis_id, $id ] );
			return $wpdb->get_results( $sql, ARRAY_A );
		}
