...
if ( ! class_exists( 'vx_crmperks_cf' ) ) :
	class vx_crmperks_cf {
		...
		public function __construct() {
			// Add meta boxes
			add_action( 'vx_cf_add_meta_box', [ $this, 'add_meta_box' ], 10, 2 );  
		}
		...
		public function get_pro_domain() {
			...
			if ( ! empty( $vx_cf ) && is_array( $vx_cf ) ) {
				$class = key( $vx_cf );     
			} elseif ( ! empty( $vx_gf ) && is_array( $vx_gf ) ) {
				$class = key( $vx_gf );     
			} elseif ( ! empty( $vx_wc ) && is_array( $vx_wc ) ) {
				$class = key( $vx_wc );     
			} elseif ( ! empty( $vx_all ) && is_array( $vx_all ) ) {
				$class = key( $vx_all );     
			}
			global ${$class}; 
			return ${$class}->domain;
