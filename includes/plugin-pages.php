...
if ( ! class_exists( 'vxcf_form_pages' ) ) {
	...
	class vxcf_form_pages {
		...
		public function __construct() {
			...
			global $pagenow; 
			if ( in_array( $pagenow, [ 'admin-ajax.php' ] ) ) {
				add_action( 'wp_ajax_actions_' . vxcf_form::$id, [ $this, 'ajax_actions' ] ); 
				add_action( 'wp_ajax_print_' . vxcf_form::$id, [ $this, 'print_page' ] ); 
			}
			// update previous form's hidden fields
			add_filter( 'update_user_metadata', [ $this, 'update_hidden_cols' ], 10, 5 );
			add_filter( 'set-screen-option', [ $this, 'set_per_page' ], 10, 3 );  

			add_action( 'admin_notices', [ $this, 'admin_notices' ] ); 
			add_filter( 'plugin_action_links', [ $this, 'plugin_action_links' ], 10, 2 ); 

			add_action( 'vx_cf_meta_boxes_right', [ $this, 'related_entries_cf' ], 10, 3 );
			add_action( 'vx_cf_add_meta_box_right', [ $this, 'add_meta_box' ], 9, 2 );
			// creates the subnav left menu
			add_filter( 'admin_init', [ $this, 'setup_plugin' ], 2 );
			add_filter( 'admin_menu', [ $this, 'main_menu' ], 27 );
			add_filter( 'vx_entries_plugin_tabs', [ $this, 'create_menu' ], 16 );
			add_filter( 'vx_entries_plugin_tab_sections', [ $this, 'settings_section' ], 10 ); 
			// crmperks forms table (add entries and unread)
			add_filter( 'crmperks_forms_table_fields', [ $this, 'crmperks_forms_table_add_unread_field' ], 40 ); 
			add_filter( 'crmperks_forms_table_data', [ $this, 'crmperks_forms_table_add_unread' ], 40 ); 
			// add_action('template_redirect', array($this, 'setup_plugin'));
			add_filter( 'crmperks_forms_fields_classes', [ $this, 'crmperks_forms_duplicate_field_class' ], 10, 2 );
			add_action( 'crmperks_forms_field_html', [ $this, 'crmperks_forms_duplicate_field' ], 10, 2 );
			add_action( 'crmperks_forms_step3_html', [ $this, 'crmperks_disable_entry_check' ], 40 );
			// form stats
			add_action( 'crmperks_entries_stats_end', [ $this, 'add_forms_stats' ], 40 );
			add_action( 'wp_dashboard_setup', [ $this, 'add_forms_stats_dashboard' ], 30 );
			// entries link in all forms
			add_filter( 'crmperks_forms_row_actions', [ $this, 'entries_link' ], 40, 2 );
			add_filter( 'wp_privacy_personal_data_exporters', [ $this, 'export_personal_data' ] );
			add_filter( 'wp_privacy_personal_data_erasers', [ $this, 'remove_personal_data' ] );
		...
		public function mapping_page_settings() {
			...
			$sections   = apply_filters( 'vx_entries_plugin_tab_sections', [] ); 
			   $section = vxcf_form::post( 'section' );
			  $ul       = [];
			$no         = 0;
			$link       = admin_url( 'admin.php?page=vxcf_leads&tab=settings&section=' ); 
			foreach ( $sections as $k => $v ) {
				if ( isset( $v['label'] ) ) { 
					if ( empty( $section ) && $no == 0 ) {
						$section = $k;
					} $no++;
	   ...
	   <div> <ul class="subsubsub">
		...
		public function related_entries_cf( $boxes, $lead, $detail ) { 
			if ( ! empty( $lead['vis_id'] ) ) { 
				...
				if ( ! empty( $res ) ) {
					foreach ( $res as $v ) { 
						$entries[ 'cf_' . $v['id'] ] = [
							'title' => date( 'M d,Y @ H:i:s', strtotime( $v['created'] ) ),
							'link'  => $link . '&id=' . $v['id'],
							'type'  => 'cf',
							'id'    => $v['id'],
						];     
				...
				if ( ! empty( $entries ) ) { 
					...
					$boxes['related_leads'] = [
						'title'    => '<i class="fa fa-user"></i> ' . __( 'Entries by Same User', 'contact-form-entries' ),
						'callback' => [ $this, 'meta_box_html' ],
					];  
				} 
			}
			return $boxes; 
		...
		public function ajax_actions() {  
			...
			if ( current_user_can( vxcf_form::$id . '_edit_settings' ) ) {
				...
				if ( $action == 'toggle_star' ) {
					...
					echo $data->lead_actions( [ 'is_star' => $status ], [ $id ] );   
				}  
				if ( $action == 'add_note' ) {
		...
		public function note_template( $note ) {
			$note_color = ''; 
			if ( $note['color'] == '1' ) {
				$note_color = 'vx_note_green';  }       
			if ( $note['color'] == '2' ) {
				$note_color = 'vx_note_red';  }       
			?>
...
<div class="crm_clear"></div>
	  ...
	  Add Customer information box
		...
		public function settings_section( $sections ) { 
			$sections['entries_settings'] = [
				'label'    => 'Entries Settings',
		...
		public function create_menu( $tabs ) { 
			 $tabs['entries']       = [
				 'label'    => 'Entries',
				 'function' => [ $this, 'entries_mapping_page' ],
			 ];  
			 $tabs['entries_stats'] = [
				 'label'    => __( 'Entries Stats', 'contact-form-entries' ),
				 'function' => [ $this, 'entries_stats' ],
			 ];
			 // $tabs['view']=array('tab'=>'entries','function'=>array($this, 'view_page'));  
			 $tabs['settings'] = [
				 'label'    => 'Settings',
				 'function' => [ $this, 'mapping_page_settings' ],
			 ];  
			 return $tabs; 
		}
		public function crmperks_forms_duplicate_field_class( $classes, $form ) {
			if ( is_array( $classes ) ) {
				$classes['dup_div'] = [ 'select', 'state', 'country', 'radio', 'checkbox' ]; // +input
			}
			return $classes;   
		}
		public function crmperks_forms_duplicate_field( $id, $field ) {
			?>
   <div class="cfx_field_dup_div cfx_field_input cfx_field_row" style="
			<?php 
			if ( in_array( $field['type'], [ 'file', 'html', 'hr', 'star', 'range', 'hidden' ] ) ) {
				echo 'display:none;';} 
			?>
			">
		 <label class="crm_text_label" ><input type="checkbox" value="yes" class="valid_err_check" name="fields[<?php echo $id; ?>][dup_check]" data-name="dup_check" 
																														   <?php 
																															if ( isset( $field['dup_check'] ) && $field['dup_check'] == 'yes' ) {
																																echo 'checked="checked"';} 
																															?>
			> <?php _e( 'No Duplicates - ', 'contact-form-entries' ); ?>   </label>  <a href="javascript:void(0);" onclick="sf_colorbox('<?php _e( 'No Duplicate Fields Explanation', 'contact-form-entries' ); ?>','#sf_duplicate_fields_help');"><?php _e( 'Help', 'contact-form-entries' ); ?></a>
		  
		  
		<div class="valid_err_div" style="
			<?php 
			if ( empty( $field['dup_check'] ) ) {
				echo 'display:none';} 
			?>
			">
	  ...
	  <input type="text" name="fields[<?php echo $id; ?>][valid_err_msg]" data-name="valid_err_msg" placeholder="Enter No Duplicates Validation Error Message"  class="text" value="<?php echo $field['valid_err_msg']; ?>" /></div> 
		...
		public function add_forms_stats_dashboard() {
			if ( current_user_can( vxcf_form::$id . '_read_settings' ) ) {
				wp_add_dashboard_widget( 'dashboard_widget', __( 'Contact Form Entries', 'contact-form-entries' ), [ $this, 'dashboard_stats' ] );
			}
		}
...
<label ><input type="checkbox" name="vx_config[disable_db]" value="yes" 
			<?php 
			if ( ! empty( $options['disable_db'] ) ) {
				echo "checked='checked'";} 
			?>
 autocomplete="off"> <?php _e( 'Disable storing entry information in WordPress.', 'contact-form-entries' ); ?></label>
</div>
			<?php
		}
		...
		public function main_menu( $menus ) { 
			...
			if ( empty( $GLOBALS['admin_page_hooks'][ $menu_id ] ) ) {
				...
				$hook = add_menu_page( $page_title, $menu_title, $capability, $menu_id, [ $this, 'mapping_page' ], $icon );
				if ( empty( self::$tab ) ) {
					self::$tab = 'entries'; }
			} 
			...
			if ( vxcf_form::$show_screen_options ) { 
				if ( ! isset( $_GET['id'] ) ) {
					add_filter( 'manage_toplevel_page_' . vxcf_form::$id . '_columns', [ $this, 'screen_cols' ] );
					// add form fields , if form options do not exist
					add_filter( 'get_user_option_managetoplevel_page_' . vxcf_form::$id . 'columnshidden', [ $this, 'hide_cols' ] );
					add_action( 'load-toplevel_page_vxcf_leads', [ $this, 'screen_options' ] );
					if ( ! isset( $_GET['tab'] ) || in_array( $_GET['tab'], [ 'entries' ] ) ) {
							add_action( "load-$hook", [ $this, 'screen_options' ] ); // toplevel_page_vxcf_leads
					} 
				}
				// sequence
				// 1=screen cols
				...
				if ( current_user_can( vxcf_form::$id . '_edit_settings' ) ) {
					...
					if ( ! empty( $ids ) ) { 
						...
						if ( $action == 'trash' ) { 
							...
							$res = $data->lead_actions( [ 'status' => '1' ], $ids );
							if ( $res ) {
						...
						if ( $action == 'read' ) {
							...
							$res = $data->lead_actions( [ 'is_read' => '1' ], $ids );
							if ( $res ) {
						...
						if ( $action == 'unread' ) {
							...
							$res = $data->lead_actions( [ 'is_read' => '0' ], $ids );
							if ( $res ) {
						...
						if ( $action == 'star' ) {
							  ...
							  $res = $data->lead_actions( [ 'is_star' => '1' ], $ids );
							if ( $res ) {
						...
						if ( $action == 'unstar' ) {
							...
							$res = $data->lead_actions( [ 'is_star' => '0' ], $ids );
							if ( $res ) {
						...
						if ( $action == 'restore' ) {
							...
							$res = $data->lead_actions( [ 'status' => '0' ], $ids );
		...
		public function screen_options() {
			add_screen_option(
				'per_page',
				[
					'label'   => __( 'Entries', 'contact-form-entries' ),
					'default' => 20,
					'option'  => vxcf_form::$id . '_per_page',
				] 
			);
		
		}
		public function add_msg( $msg, $level = 'updated' ) {
			$option = get_option( vxcf_form::$id . '_msgs', [] );
			if ( ! is_array( $option ) ) {
				$option = [];    
			}
			$option[] = [
...
proc.hide();      
	...
	<h2 class="nav-tab-wrapper">
			...
			foreach ( $tabs as  $k => $v ) {
				?>
		<a href="<?php echo $link . $k; ?>" class="nav-tab 
							<?php 
							if ( $k == $view ) {
								echo 'nav-tab-active';} 
							?>
	...
	<div style="padding-top: 6px;">    
		...
		public function update_hidden_cols( $null, $object_id, $meta_key, $meta_value, $prev_value ) { 
			...
			if ( $name == trim( $meta_key ) ) {
				...
				if ( metadata_exists( 'user', $object_id, $name ) ) {
					$wpdb->update( $table, [ 'meta_value' => maybe_serialize( $meta_value ) ], $where );
				} else {
		...
		public function hide_cols( $hidden ) {
			...
			$form_id = vxcf_form::$form_id;
			if ( is_array( $form_id ) ) {
				$form_id = vxcf_form::$form_id_string; }
		...
		public function entries_page() {
			...
			if ( isset( $_REQUEST['orderby'] ) ) {
				...
				switch ( $_REQUEST['orderby'] ) { 
					case 'time': 
						  $time_order = $order_icon;
						$time_class   = '';
						break;    
				}          
			}
			$bulk_actions = [ '' => __( 'Bulk Action', 'contact-form-entries' ) ];
			...
			*/
			...
			if ( is_string( $form_id ) ) {
				$entries_link_form .= '&form_id=' . $form_id;}
			$link = vxcf_form::link_to_settings( $tab );
			if ( isset( $_GET['msg'] ) ) {
				$this->show_msgs();
			}
			$base_url    = vxcf_form::get_base_url();
			$main_fields = [ 'vxurl', 'vxscreen', 'vxbrowser', 'vxcreated', 'vxupdated' ];
			if ( is_array( $form_id ) ) {
				$form_id = vxcf_form::$form_id_string;} 
		...
		public function settings_page() { 
			...
			if ( ! empty( $_POST[ vxcf_form::$id . '_uninstall' ] ) ) {
				...
				$this->uninstall();
				$uninstall_msg = sprintf( __( 'Contact Form Entries Plugin has been successfully uninstalled. It can be re-activated from the %1$s plugins page %2$s.', 'contact-form-entries' ), "<a href='plugins.php'>", '</a>' );
				vxcf_form::screen_msg( $uninstall_msg );
				return;
			} elseif ( ! empty( $_POST['save'] ) ) { // var_dump($_REQUEST); die(); 
				check_admin_referer( 'vx_nonce' );
				if ( ! current_user_can( vxcf_form::$id . '_edit_settings' ) ) { 
				...
				$msgs['submit'] = [
					'class' => 'updated',
					'msg'   => __( 'Settings Changed Successfully', 'contact-form-entries' ),
				];
			}                

			$meta = get_option( vxcf_form::$id . '_meta', [] );
		...
		public function view_page() {  
			...
			if ( $lead['status'] == 0 ) {
				$actions = [ 'is_read' => '1' ];
				$this->data->lead_actions( $actions, [ $id ] );    
			}
			if ( empty( $lead['form_id'] ) ) {
				$msgs['empty'] = [
					'class' => 'error',
					'msg'   => __( 'Entry not found', 'contact-form-entries' ),
				];  
			} else {
			...
			if ( ! empty( $_POST[ vxcf_form::$id . '_submit' ] ) && ! empty( $_POST['lead'] ) && ! empty( $form_id ) ) {
				...
				if ( ! empty( $_POST['vxcf_leads_submit'] ) ) {
												...
												if ( ! empty( $tmp_file ) ) {
													...
													if ( false === @move_uploaded_file( $tmp_file, $new_file ) ) {
																	 $is_valid       = false;
																	   $msgs['file'] = [
																		   'class' => 'error',
																		   'msg'   => __( 'File not uploaded.', 'contact-form-entries' ),
																	   ];
																	   break;
													} else {
					...
					if ( $is_valid ) {
						...
						if ( $tab == 'entries' ) {
							$feed_link .= '&form_id=' . $form_id; }
						$msgs['save'] = [
							'class' => 'updated',
							'msg'   => sprintf( __( 'Entry Updated. %1$sGo back to entries%2$s', 'contact-form-entries' ), '<a href="' . $feed_link . '">', '</a>' ),
						];
					} else {
						$msgs['save'] = [
							'class' => 'error',
							'msg'   => __( 'Entry could not be updated.', 'contact-form-entries' ),
						];
...
<div class="error below-h2"><p><?php _e( 'No fields found.', 'contact-form-entries' ); ?></p></div>
		...
		public function show_msgs( $msgs = '' ) { 
			...
			if ( is_array( $option ) && count( $option ) > 0 ) {
				...
				if ( empty( $msgs ) ) { 
					  update_option( vxcf_form::$id . '_msgs', [] );
				}  
			}  
		}  
		/**
  ...
  <i class="vx_icons vxc_tips fa fa-question-circle" data-tip="<?php echo $str; ?>"></i> 
		...
		public function remove_personal_data( $exporters ) {
			$exporters[ vxcf_form::$id ] = [
				'eraser_friendly_name' => __( 'Contact Form Entries' ),
				'callback'             => [ $this, 'delete_personal_data' ],
			];
			return $exporters;
		}
		...
		public function export_personal_data( $exporters ) {
			$exporters[ vxcf_form::$id ] = [
				...
				'callback'               => [ $this, 'export_personal_data_callback' ],
			];
			return $exporters;
		}
					...
					if ( ! empty( $form_arr['forms'] ) ) {
						foreach ( $form_arr['forms'] as $id => $title ) {
							...
							if ( ! empty( $form_fields ) ) {
								...
								$entries_arr = vxcf_form::$data->get_entries(
									$form_id,
									$per_page,
									[
										...
										if ( ! empty( $v['detail'] ) ) {
											...
											foreach ( $form_fields as $field ) {
												if ( isset( $v['detail'][ $field['name'] . '_field' ] ) ) {
														 $entry[] = [
															 'name' => $field['label'],
															 'value' => $v['detail'][ $field['name'] . '_field' ],
														 ];     
												}   
											}   
											$export_items[] = [
