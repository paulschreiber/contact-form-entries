...
if ( ! class_exists( 'vx_crmperks_notice_cf_form' ) ) :
	class vx_crmperks_notice_cf_form {
		...
		public function __construct() {
			add_filter( 'vx_entries_plugin_tabs', [ $this, 'add_section_cf' ], 99 );
			add_filter( 'plugin_row_meta', [ $this, 'pro_link' ], 10, 2 );
			// add_action( 'after_plugin_row_'.vxcf_form::get_slug(), array( $this, 'plugin_msgs' ),99,3 );
			add_action( 'wp_ajax_vxcf_form_review_dismiss', [ $this, 'review_dismiss' ] );
			add_action( 'add_section_vxcf_leads', [ $this, 'free_plugins_notice' ], 99 );

			if ( isset( $_GET['page'] ) && $_GET['page'] == vxcf_form::$id ) {
				add_filter( 'admin_footer_text', [ $this, 'admin_footer' ], 1, 2 );
				// install forms
				add_action( 'admin_notices', [ $this, 'install_forms_notice' ] );
			}
			add_filter( 'plugins_api', [ $this, 'forms_info' ], 11, 3 ); 
		}
		...
		public function install_forms_notice() {
			...
			if ( empty( $show ) ) {        
				...
				if ( ! class_exists( 'cfx_form' ) ) {
					...
					$msg = sprintf( __( '%1$sCRM Perks Forms Plugin%2$s is a free lightweight contact form builder and designer with floating buttons and popups.', 'contact-form-entries' ), '<a href="https://wordpress.org/plugins/crm-perks-forms/" target="_blank">', '</a>' );
					?>
...
<p><a href="<?php echo $url; ?>"><?php echo $plugin_msg; ?></a> | <a href="<?php echo $link; ?>"><?php _e( 'Dismiss this notice', 'contact-form-entries' ); ?></a></p>
  ...
  <div style="background-color: rgba(224, 224, 224, 0.5);  padding: 5px; margin: 0px 10px 10px 28px "><div style="background-color: #d54d21; padding: 5px 10px; color: #fff" class="vx_msg"> <span class="dashicons dashicons-info"></span> <?php echo wp_kses_post( $message ); ?>
		...
		public function admin_footer( $text ) {
			if ( isset( $_GET['page'] ) && $_GET['page'] == vxcf_form::$id ) {
				$text = sprintf( __( 'if you enjoy using %1$sContact Form Entries Plugin%2$s, please %3$s leave us a %4$s rating%5$s. A %6$shuge%7$s thank you in advance.', 'contact-form-entries' ), '<b>', '</b>', '<a href="' . $this->review_link . '" target="_blank" rel="noopener noreferrer">', '&#9733;&#9733;&#9733;&#9733;&#9733;', '</a>', '<b>', '</b>' );
			} return $text;}
  ...
  <p><?php echo sprintf( __( 'You\'ve been using Contact Form Entries Plugin for some time now; we hope you love it!.%1$s If you do, please %2$s leave us a %3$s rating on WordPress.org%4$s to help us spread the word and boost our motivation.', 'contact-form-entries' ), '<br/>', '<a href="' . $this->review_link . '" target="_blank" rel="noopener noreferrer">', '&#9733;&#9733;&#9733;&#9733;&#9733;', '</a>' ); ?></p>
 ...
 //$.ajax({ type: "POST", url: ajaxurl, async : false, data: {action:"vxcf_form_review_dismiss"} });          
		...
		public function add_section_cf( $tabs ) {
			$tabs['vxc_notice'] = [
				'label'    => __( 'Go Premium', 'contact-form-entries' ), 
				'function' => [ $this, 'notice' ],
			];
			$this->review_notice();       
