  ...
  <?php
	...
	foreach ( $counts as $label => $count ) {
		...
		if ( is_array( $count ) ) { 
			...
			$val   = $count['val'];
			$count = $count['count'];
			if ( isset( $_GET[ $field ] ) && $_GET[ $field ] == $val ) {
				$current = 'current';  }
		}    
		?>
	   <li>
...
?>
  ...
  <span style="
  <?php 
	if ( vxcf_form::post( 'time' ) != 'custom' ) {
		echo 'display:none';} 
	?>
		" class="crm_custom_range"> 
  <input type="text" name="start_date" placeholder="<?php _e( 'From Date', 'contact-form-entries' ); ?>" value="
															  <?php 
																if ( isset( $_REQUEST['start_date'] ) ) {
																	echo $_REQUEST['start_date'];} 
																?>
		" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="
  <?php 
	if ( isset( $_REQUEST['end_date'] ) ) {
		echo $_REQUEST['end_date'];} 
	?>
		" placeholder="<?php _e( 'To Date', 'contact-form-entries' ); ?>" name="end_date"  style="width: 100px">
  </span>
...
foreach ( $fields as $field ) {  
	...
	?>
  <th scope="col" class="manage-column vx_sort_sql vx_sort column-<?php echo $field['_id'] . $hide_col; ?>" data-sort="string" id="<?php echo $field['_id']; ?>"  data-name="<?php echo $field['name']; ?>" 
																			 <?php 
																				if ( isset( $field['vx_width'] ) ) {
																					echo 'style="width: ' . $field['vx_width'] . '"';} 
																				?>
...
?>
  ...
  <tr class='
			<?php 
			echo 'vx_lead_' . $lead['type'];
			if ( $lead['is_read'] == 0 ) {
				echo ' vx_lead_unread';
			}  if ( $lead['type'] == '1' ) {
				echo '  vx_';} 
			?>
	' id="tr_<?php echo $lead['id']; ?>" data-id="<?php echo $lead['id']; ?>" >
	...
	<i class="fa fa-star toggle_star <?php echo $lead['is_star'] == 1 ? 'crm_star_yellow' : 'crm_star_black'; ?>"></i> </td>
			...
			foreach ( $fields as $field_id => $field ) {   
				...
				if ( in_array( $field['name'], $main_fields ) ) {
					...
					if ( in_array( $main_field, [ 'created', 'updated' ] ) ) {
						$field_label = strtotime( $lead[ $main_field ] ); // +$offset
						$field_label = date( 'M-d-Y H:i:s', $field_label );   
					}
					if ( $main_field == 'screen' ) {
						$field_label = $lead['screen']; }
					if ( in_array( $main_field, [ 'url', 'browser' ] ) ) {
				...
				if ( $f_no == 1 ) {
					...
					$mark_action  = 'read';
					$mark_label   = __( 'Mark Read', 'contact-form-entries' );
					if ( $lead['is_read'] == 1 ) {
						 $mark_action = 'unread';
						$mark_label   = __( 'Mark Unread', 'contact-form-entries' );    
					}
				...
				?>
			   <td class="column-<?php echo $field['_id'] . $hide_col; ?>">
											<?php 
											echo $field_label; 
											if ( $f_no == 1 ) {
												?>
...
<label for='vx_check_<?php echo $field['name']; ?>'><input type="checkbox" 
								<?php 
								if ( isset( $entry_fields[ $field['name'] ] ) ) {
									echo 'checked="checked"';} 
								?>
 name="fields[<?php echo $field['name']; ?>]" id='vx_check_<?php echo $field['name']; ?>' > <?php echo $field['label']; ?></label></div>
		<?php
	}
}
?>
