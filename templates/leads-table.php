...
<input type="search" placeholder="<?php _e( 'Search', 'contact-form-entries' ); ?>"  data-column="all" class="vx_search">
	...
	if ( ! empty( $export ) ) { 
		?>
		<a class="vx_export_btn" href="?vx_crm_form_action=download_csv&vx_crm_key=<?php echo $export; ?>"><?php _e( 'Download CSV', 'contact-form-entries' ); ?></a> <?php } ?>
 </p>
	<?php
}
?>
...
<th 
		<?php 
		if ( ! empty( $atts['number-col-width'] ) ) {
			echo 'style="width:' . $atts['number-col-width'] . ' "'; } 
		?>
><?php _e( '#', 'contact-form-entries' ); ?></th>
		<?php
	}
	$total_cols = 2;
