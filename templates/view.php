...
if ( is_array( $detail ) && is_array( $fields ) ) {
	foreach ( $fields as $field ) { // var_dump($field['basetype']);
		...
		if ( ! isset( $field['name'] ) || ! empty( $field['vx_skip_edit'] ) ) {
			continue; }
...
<div class="vx_edit">
		<?php
		if ( in_array( $type, [ 'textarea', 'chat', 'comma' ] ) ) {
			if ( is_array( $value ) ) {
				$value = implode( ',', $value );
			}
			?>
  ...
  <span class="vx_check_label"><?php echo $text; ?></span>
		...
		} elseif ( in_array( $type, [ 'select', 'multiselect', 'state', 'country' ] ) ) {
			if ( $type == 'state' ) {
...
<div class="vx_file_single">
 ...
 <a href="#" class="vx_del_link vx_float_right vx_remove_file"><?php _e( 'Remove', 'contact-form-entries' ); ?></a>
			...
			$settings  = [
				'textarea_name' => 'lead[' . $field['name'] . ']',
				'tinymce'       => [ 'forced_root_block' => 'div' ],
				'textarea_rows' => 20,
			];
			...
			if ( $type == 'date' ) {
				$date_field = $date_class = ' vx_date_field '; }
			$type  = ! in_array( $type, [ 'email', 'url', 'tel' ] ) ? 'text' : $type;
...
<div class="vx_value">
  ...
  <a href="mailto:<?php echo $value; ?>"><?php echo $value; ?></a>
			<?php
		} elseif ( $type == 'url' && filter_var( $value, FILTER_VALIDATE_URL ) ) { // it is a url
			?>
  <a href="<?php echo $value; ?>" target="_blank"><?php echo $value; ?></a>
			<?php
		} elseif ( $type == 'img' && filter_var( $value, FILTER_VALIDATE_URL ) ) { // it is a image
			?>
  <a href="<?php echo $value; ?>" target="_blank"><img src="<?php echo $value; ?>" class="vx_img_thumb_field" /></a>
			<?php
		} elseif ( $type == 'audio' && filter_var( $value, FILTER_VALIDATE_URL ) ) {
...
<audio controls src="<?php echo $value; ?>" style="width: 100%" ></audio>
  ...
  <a href="<?php echo $value; ?>" target="_blank"><?php echo $value; ?></a>
			<?php
		} elseif ( $type == 'video' && filter_var( $value, FILTER_VALIDATE_URL ) ) {
			?>
...
$colors = [
	'0' => __( 'Yellow Note', 'contact-form-entries' ),
	'1' => __( 'Green Note', 'contact-form-entries' ),
	'2' => __( 'Red Note', 'contact-form-entries' ),
];
?>
</div>
...
if ( ! empty( $lead['meta'] ) ) {
	...
	$pay_labels = [
		'status'     => __( 'Status', 'contact-form-entries' ),
		'total_text' => __( 'Total', 'contact-form-entries' ),
		'gateway'    => __( 'Gateway', 'contact-form-entries' ),
		'id'         => __( 'Invoice ID', 'contact-form-entries' ),
	];
	$pays       = [];
	foreach ( $pay_labels as $k => $v ) {
		if ( isset( $entry_meta['payment'][ $k ] ) ) {
			 $val = $entry_meta['payment'][ $k ];
			if ( in_array( $k, [ 'status', 'gateway' ] ) ) {
				$val = ucfirst( $val );
			} $pays[ $v ] = $val; }
	}
	$boxes = [
		'payment' => [
			'title' => __( 'Payment Details', 'contact-form-entries' ),
			'vals'  => $pays,
		],
	];
}
